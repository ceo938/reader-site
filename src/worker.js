...
const ALLOWED = ["bbc.com", "bbc.co.uk", "nytimes.com", "theguardian.com", "theverge.com", "arstechnica.com", "news.ycombinator.com", "hnrss.org"];
...
async function extract(u) {
  ...
  let title = "", inArticle = false, depth = 0, cur = null;
  const paras = [], artParas = [];
  const rw = new HTMLRewriter()
    ...
    .on("p", {
      element(e) { cur = { t: "", a: inArticle }; e.onEndTag(() => { const s = cur.t.replace(/\s+/g, " ").trim(); if (s.length > 40) (cur.a ? artParas : paras).push(s); cur = null; }); },
      text(t) { if (cur) cur.t += t.text; },
    })
    .on("script,style,nav,footer,aside", { element(e) { e.remove(); } });
  await rw.transform(r).text();
  const body = (artParas.length >= 3 ? artParas : paras).slice(0, 60);
  return { title: title.trim(), paras: body };
}
...
async function read(url, env) {
  ...
  const { title, paras } = await extract(u);
  ...
  const result = { title, ko_title: p.ko_title, paras: paras.map((en, i) => ({ en, ko: p.paras[i] || "" })), url: u };
